...
import { createMeeting, getToken, validateMeeting } from "../../api1.jsx";
      ...
      <>
        <div>
          <button
            ref={btnRef}
            onClick={onClick}
            style={{backgroundColor: `${onState ? "bg-white" : "rgb(234,86,86)"}`}}
            className={`rounded-full min-w-auto w-12 h-12 flex items-center justify-center 
            ${onState ? "bg-white" : "text-white"}`}
          >
            {onState ? (
  ...
  return (
    <div>
      <div className="overflow-y-auto flex flex-col flex-1 h-screen">
        <button
          onClick={() => navigate('/livesessions/history')}
          className="absolute top-4 right-4 bg-red-700 text-white px-4 py-2 rounded-lg shadow hover:bg-red-900 z-10"
        >
